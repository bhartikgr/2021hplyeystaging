...
(function ($) {
  ...
  if ($(window).width() < 600) {
    $("#listeo_core-search-form div.row:nth-child(2) div.panel-wrapper").append(
      "<button type='button' class='cristian_more_filter'>Filters</button>"
    );
    $("body").prepend(
      "<div class='cristian_more_filter_modal'>" +
  ...
  $(".sort-by").appendTo(
    "#listeo_core-search-form div.row:nth-child(2) div.panel-wrapper"
  );
  ...
  $(".cristian_more_filter").click(function () {
    $("#listeo_core-search-form").prependTo(
      ".cristian_more_filter_modal .modal_body"
    );
    $("body").css("overflow", "hidden");
    $(".cristian_more_filter_modal").css({
      display: "block",
  ...
  $(document).ready(function () {
    ...
    var pos = $(".listing-manager-error p")
      .text()
      .indexOf("is a required field");
    // console.log($(".listing-manager-error p").text().substring(0,pos-1));
    if (pos > 0) {
      if ($(".listing-manager-error p").text().indexOf("Pricing") >= 0)
        setTimeout(function () {
          $("div.add-listing-section.menu").attr("tabindex", -1).focus();
        }, 2000);
      else if ($(".listing-manager-error p").text().indexOf("Gallery") >= 0)
        setTimeout(function () {
          $("div.add-listing-section.gallery").attr("tabindex", -1).focus();
        }, 2000);
    }
    ...
    if (
      window.history &&
      history.pushState &&
      $("form#listing_preview").length
    ) {
      addEventListener("load", function () {
        history.pushState(null, null, null); // creates new history entry with same URL
        addEventListener("popstate", function () {
          var leavePage = confirm(
            "If you go back, your previous settings would be unsaved. Do you still want to leave this page?"
          );
          if (leavePage) {
            history.back();
          } else {

	...
	public function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			if(isset($action) && $action == 'archive') {
				echo do_shortcode('[listeo_search_form dynamic_filters="'.$dynamic.'" 	more_text_open="'.esc_html__('More Filters','listeo_core').'" more_text_close="'.esc_html__('Close Filters','listeo_core').'" action='.get_post_type_archive_link( 'listing' ).']');
			} else {
	...
	public function widget( $args, $instance ) {
		...
		<div class="row with-forms  margin-top-0" id="booking-widget-anchor" >
			<form id="form-booking" data-post_id="<?php echo $post_info->ID; ?>" class="form-booking-<?php echo $post_meta['_listing_type'][0];?>" action="<?php echo esc_url(get_permalink(get_option( 'listeo_booking_confirmation_page' ))); ?>" method="post">
					...
					<div class="col-lg-12">
						<input type="text" class="time-picker flatpickr-input active" placeholder="<?php esc_html_e('Time','listeo_core') ?>" name="_hours" readonly="readonly">
					</div>
...
<?php if(isset($service['bookable_quantity'])) : ?> <?php endif; ?>
					...
					?>
					<!-- Panel Dropdown -->
					<!-- <div class="col-lg-12" <?php if($max_guests == 1 || $count_per_guest != "on"){ echo 'style="display:none;"'; } ?>>
						<div class="panel-dropdown">
							<a href="#"><?php esc_html_e('People','listeo_core') ?> <span class="qtyTotal" name="qtyTotal">1</span></a>
...
class Listeo_Core_Owner_Widget extends Listeo_Core_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'listeo_core widget_listing_owner boxed-widget my_widget1';
...
<p>To protect your payment, never exchange money or communicate outside of the Hypley website.

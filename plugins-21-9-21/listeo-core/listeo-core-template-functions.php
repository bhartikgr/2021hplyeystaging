...
function get_listing_bookmarks_number( $post = null ) {
	return Listeo_Core_Listing::get_listing_bookmark_number( $post );
}
...
function arlisteo_core_ajax_pagination($vpages = '',$uvpages = '', $current = false, $range = 2 ) {
	$pages = $vpages+$uvpages;
	if($vpages < $current){
		$current = $current - $vpages;
	}
